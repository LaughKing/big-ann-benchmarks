...
# 修改输出目录以反映新内容
output_dir = "throughput_trajectory_plots_custom_style"
...
# 3. 识别 batchThroughput 列
# 修改正则表达式以匹配 'batchThroughput_'
throughput_cols = sorted([col for col in data.columns if re.match(r'batchThroughput_\d+', col)], 
                         key=lambda x: int(x.split('_')[1]))

# --- 更新的诊断信息 ---
if not throughput_cols:
    print("错误：在文件中没有找到 'batchThroughput_N' 格式的列。")
    exit()
else:
    # 打印找到的列数
    print(f"诊断信息：在文件中找到了 {len(throughput_cols)} 个 'batchThroughput' 数据点 (X轴范围将是 0 到 {len(throughput_cols)-1}).")
...
for dataset_name in unique_datasets:
    ...
    for index, row in subset_df.iterrows():
        ...
        # 使用 throughput_cols
        y_values = row[throughput_cols].values.astype(float)
    ...
    # --- 应用专业的图表格式 ---
    # 更新Y轴标签和标题
    plt.xlabel("Batches", fontproperties=LABEL_FP)
    plt.ylabel("Batch Throughput (QPS)", fontproperties=LABEL_FP)
    plt.title(f"Algorithm Throughput Trajectory on '{dataset_name}'", fontproperties=LABEL_FP, weight=LABEL_WEIGHT, pad=20)
    ...
    save_path = os.path.join(output_dir, f"throughput_trajectory_{safe_dataset_name}.pdf")
